...
TELEGRAM_BOT_TOKEN = "8550463664:AAGq23HF3tX-82Fy8E69eeHDquX7J9ug6rE" # Change with your telegram bot token
TELEGRAM_CHAT_ID = "7575574860" # Change with your telegram chat id

# Configure requests session to ignore SSL verification
session = requests.Session()
session.verify = False

# Configure telebot to use custom session
...
def load_notification_counts():
    if os.path.exists(NOTIFICATION_COUNT_FILE):
        try:
            with open(NOTIFICATION_COUNT_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except:
            return {}
    return {}
...
def save_notification_counts(counts):
    with open(NOTIFICATION_COUNT_FILE, 'w', encoding='utf-8') as f:
...
def save_cookies(cookies):
    ...
    with open('cookies/cookies.json', 'w', encoding='utf-8') as f:
...
def startmonitor_command(message):
    ...
    MONITOR_THREAD = threading.Thread(target=monitor_wishlist, args=(message.chat.id,), daemon=True)
...
def fetch_page(cookies, page_num):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            # Use session with SSL verification disabled
            response = session.get(WISHLIST_API, params=params, cookies=cookies, headers=headers, timeout=REQUEST_TIMEOUT)
...
def monitor_wishlist(chat_id=None):
    global PREVIOUS_STOCK_STATUS, NOTIFICATION_COUNTS, MONITORING_ACTIVE
...
"""
    ...
    logger.info(f"Created by: @brocodx")
    logger.info(f"Channel: @brocodx")
...
""")
    ...
    logger.info("Created by: @brocodx")
    logger.info("Channel: @brocodx")
